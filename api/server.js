...
app.post('/api/register', async (req, res) => {
  try {
    ...
    currentLocalDB.push({ ...newProfile, contactNumber: reg.contactnumber, createdAt: newProfile.createdat });
    saveRegistrations(currentLocalDB);
    return res.json({ success: true });
  } catch {
    res.status(500).json({ success: false, error: 'Database exception.' });
  }
});
...
app.post('/api/posts', async (req, res) => {
  try {
    ...
    return res.json({ success: false, error: 'Database unlinked.' });
  } catch (err) {
